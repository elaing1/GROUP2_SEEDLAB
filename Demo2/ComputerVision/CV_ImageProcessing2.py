...
Image Processing
"""
def ImageProcessing(q, stop, ser, ReadfromArduino):
    start = time.time()
    error = 0
    ...
    while stop is not True:
        ...
        if (ids is not None) and ((time.time() - start) > 3):
            start = time.time()
...
image_proc = [Timer(n*1, ImageProcessing, args=(q, video_stream.stopped, ser, ReadfromArduino, )) for n in range(4)]
